...
function isNightTime(sunriseStr, sunsetStr, nowStr) {
...
async function getWeather(city) {
  try {
    ...
    const sunriseToday    = daily.sunrise && daily.sunrise[0];
    const sunsetToday     = daily.sunset  && daily.sunset[0];
    const sunriseTomorrow = daily.sunrise && daily.sunrise[1];
    const nowAtLocation   = current.time;

    const night = isNightTime(sunriseToday, sunsetToday, nowAtLocation);
    ...
    return {
      ...
      sun: { sunriseToday, sunsetToday, sunriseTomorrow, now: nowAtLocation },
...
async function showWeather(city) {
  ...
  if (data.sun && window.setSunTimes) {
    window.setSunTimes(
      data.sun.sunriseToday,
      data.sun.sunsetToday,
      data.sun.sunriseTomorrow,
      data.sun.now
    );
  }
